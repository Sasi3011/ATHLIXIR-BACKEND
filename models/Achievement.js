...
router.get('/', auth, async (req, res) => {
  try {
    console.log('Fetching achievements for athlete:', req.user.email);
...
router.post('/', auth, [
  ...
  console.log('POST request to /api/achievements received');
  ...
  try {
    ...
    console.log('Creating new achievement for:', req.user.email);
    ...
    console.log('Achievement created with ID:', achievement._id);
...
// Add this debug route to test if the achievements router is working
router.get('/test/route', (req, res) => {
  res.status(200).json({ message: 'Achievements route is working' });
});
